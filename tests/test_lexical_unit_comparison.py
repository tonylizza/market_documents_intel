...
def test_metrics_match_direct_calls_to_existing_metric_functions():
    ...
    assert metrics.tfidf_cosine == pairwise_tfidf_cosine_similarity(tokens_a, tokens_b)
    ...
    assert metrics.edit_similarity == character_edit_similarity(EARLIER, LATER)
...
def test_undefined_metrics_stay_none_not_fabricated():
    ...
    assert metrics.tfidf_cosine is None
...
def test_identical_texts_produce_identical_similarity_of_one():
    ...
    assert metrics.tfidf_cosine == 1.0
